...
export const useCuestionarioStore = create(
    persist(
        (set) => ({
            ...
            setSelectedCuestionario: (cuestionario) => set({ selectedCuestionario: cuestionario }),
            clearSelectedCuestionario: () => set({ selectedCuestionario: null }),
            updateCurrentCuestionario: (data) => set((state) => ({
                currentCuestionario: { ...state.currentCuestionario, ...data },
            })),
            resetCurrentCuestionario: () => set({
                currentCuestionario: {
                    id: Date.now(),
                    nombre: '',
                    fechaCreacion: 0,
            ...
            setRespuesta: (idCuestionarioOrganizacion, idItem, valor_opcion_seleccionada, observaciones) =>
                set((state) => {
                    // Verifica si la respuesta ya existe en el estado
                    const existingRespuesta = state.respuestas.find(
                        (respuesta) => respuesta.idItem === idItem && respuesta.idCuestionarioOrganizacion === idCuestionarioOrganizacion
                    );
                    ...
                    if (existingRespuesta) {
                        return {
                            respuestas: state.respuestas.map((respuesta) =>
                                respuesta.idItem === idItem && respuesta.idCuestionarioOrganizacion === idCuestionarioOrganizacion
                        ...
                        return {
                            respuestas: [
                                ...state.respuestas,
                                { idCuestionarioOrganizacion, idItem, valor_opcion_seleccionada, observaciones },
                            ],
                        };
                    }
                }),
